...
{
 public:
  virtual int valueNumber() { return 0; }
};
...
class Instruction
{
 ...
 public:
 Instruction(Value *v, Instruction *p, Instruction *n) :value (v), previous(p), next(n), substitute(0) { }
  ...
  void redirectTo(Instruction *other) { substitute = other; }
  void setNext(Instruction *n) { next = n; }

  // accessors for next and previous
  Instruction * getNext() { return next; }
  void setPrevious (Instruction *p) { previous = p; }
  Instruction * getPrevious() { return previous; }
...
protected:
 ...
 BinaryInstruction( Value *value, Instruction *oper0, Instruction *oper1)
  : operand0(oper0),
    operand1(oper1), Instruction(value, 0, 0)
    { }
...
class Add : public BinaryInstruction
{
 ...
 Add(Instruction *operand0, Instruction *operand1) :
  ...
  virtual void accept(InstructionVisitor &v);
  virtual void print() { cout << " ADD "; operand0->print(); operand1->print(); }
};
...
class Mul : public BinaryInstruction
{
 public:
  ...
  virtual void accept(InstructionVisitor &v);
  virtual void print() { cout << " MUL "; operand0->print(); operand1->print(); }
...
{
 public:
 Constant(Value *value) : Instruction(value, 0, 0) { }
  ...
  virtual void visitOperands(OperandVisitor &v);
  virtual void accept(InstructionVisitor &v);
  virtual void print() { cout << " CONSTANT " << value->valueNumber() << " "; }
};
...
class LocalVariable : public Instruction
{
 ...
 public:
 LocalVariable(int slotNumber) : Instruction(0, 0, 0) {
    this->slotNumber = slotNumber;
  }

 LocalVariable(int slotNumber, Value *value) : Instruction(value, 0, 0) {
    this->slotNumber = slotNumber;
  }

  virtual void accept(InstructionVisitor &v);
  virtual void visitOperands(OperandVisitor &v);
  virtual void print() { cout << " v" << slotNumber << " "; }
};
...
class Move : public Instruction
{
 ...
 public:
 Move(LocalVariable *var, Instruction *rValue) :Instruction (0,0,0) {
    rightValue = rValue;
    variable = var;
  }
  ...
  virtual void visitOperands(OperandVisitor &v);
  virtual void accept(InstructionVisitor &v);
  virtual void print() { variable->print(); cout << " <- "; rightValue->print(); }
};
